...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `
    <div class="single-product text-white">
      <div>
    <img class="product-image" src=${image}></img>
      ...
      <button onclick="showDetails(${product.id})" id="details-btn" class="btn btn-info">Details</button></div>
    `;
...
//details button
const showDetails = (idNumber) => {
  document.getElementById("detail-text").textContent = '';
  const data = allData;
  const num = data[idNumber - 1].id;
  const detailData = num - 1;
  const div = document.createElement('div');
  div.classList.add("show-details");
  div.innerHTML = `
  <p>Details:${data[detailData].id}. ${data[detailData].description} </p>
    `;
